...
function safeFoodFirst(
  ...
  // Spur-stripped room after eat — lower bars so mid/long snakes still take food
  // (mobility-filter era + len*0.2/0.35 made them circle forever)
  if (snake.length >= 40) {
    if (qAfter.effective < Math.max(6, Math.floor(snake.length * 0.12))) return null
    if (qAfter.oddSpurs > 0 && qAfter.effective < snake.length * 0.22) return null
  } else if (snake.length >= 20) {
    if (qAfter.effective < Math.max(6, Math.floor(snake.length * 0.15))) return null
  }
  // Keep extreme-long guard soft
  if (snake.length >= 140 && qAfter.effective < Math.floor(snake.length * 0.1)) return null
...
function evaluate(
  ...
  if (rt && rq.effective > len * 1.3 && moves.length >= 2) score -= fd * 110
  else if (rt && rq.effective > len * 1.0) score -= fd * 50
  else if (rt) score -= fd * 15
  else score -= fd * 3
